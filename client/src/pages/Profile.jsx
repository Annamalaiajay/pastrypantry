...
function Profile({ colors }) {
  ...
  useEffect(() => {
    ...
    // If name exists, update state. If not, keep "Guest"
  ...
  const calculateTier = () => {
    ...
    if (joinDate) {
        ...
        if (diffDays <= 30) {
        return { label: "New User 🌱", color: "#4CAF50" }; 
        }
    }
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto', fontFamily: 'Helvetica, sans-serif', color: colors.text }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '40px', paddingBottom: '20px', borderBottom: `2px solid ${colors.accent}` }}>
        ...
        <div>
          {/* --- FIXED: NOW SHOWS REAL NAME --- */}
          <h1 style={{ margin: 0, textTransform: 'capitalize' }}>{userName}</h1>
          
          <p style={{ margin: '5px 0', opacity: 0.7 }}>Member since {joinDate ? joinDate.toLocaleDateString() : 'Today'}</p>
          
          {/* --- FIXED: NOW SHOWS REAL STATUS --- */}
          <span style={{ 
            backgroundColor: status.color, 
      ...
      {orders.length === 0 ? (
        <p style={{ opacity: 0.6 }}>No orders yet. Place an order to see it here!</p>
      ) : (
